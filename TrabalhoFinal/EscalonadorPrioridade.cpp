...
EscalonadorPrioridade::EscalonadorPrioridade(const std::vector<Processo>& procs, int ram, int politica, int tamFrame)
    : ram(ram), politicaMem(politica), tamFrame(tamFrame) {
    processosIniciais = procs;
}

void EscalonadorPrioridade::executarSimulacao() {

    GerenciadorMemoria gerenciadorMemoria(ram, politicaMem, tamFrame);

    std::vector<ProcessoPrioridade> lista;
    int idContador = 1;
    ...
    std::cout << "\n==============================================" << std::endl;
    std::cout << "    INICIANDO SIMULACAO COMPLETA (PRIO + MEM) " << std::endl;
    std::cout << "==============================================" << std::endl;

    while (processosConcluidos < totalProcessos) {
        int indiceMelhorProcesso = -1;
        int maiorPrioridade = INT_MAX; 

        for (int i = 0; i < totalProcessos; i++) {
            if (lista[i].tempoChegada <= tempoAtual && lista[i].tempoRestante > 0) {
                if (lista[i].prioridade < maiorPrioridade) {
                    maiorPrioridade = lista[i].prioridade;
        ...
        ProcessoPrioridade& pAtual = lista[indiceMelhorProcesso];
        ...
        std::cout << "[Tempo " << tempoAtual << "] CPU executando P" << pAtual.id 
                  << " (Prioridade: " << pAtual.prioridade << ") | Restante: " << pAtual.tempoRestante << " u.t.\n";
    ...
    std::cout << "\n=======================================================\n";
    std::cout << "         RELATORIO DE PERFORMANCE (PRIORIDADE)         \n";
    std::cout << "=======================================================\n";
