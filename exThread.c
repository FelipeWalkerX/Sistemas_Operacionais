#include <pthread.h>
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>

#define NUM_THREADS 5

int saldo = 1000;
pthread_mutex_t mutex;

void *AtualizaSaldo(void *n) {
    int num = (intptr_t)n; 
    
    int meu_saldo = saldo;
    int novo_saldo = meu_saldo + (num * 100); 
    
    pthread_mutex_lock(&mutex);
        printf("Sou a Thread %d. Novo saldo: %d\n", num, novo_saldo);
    pthread_mutex_unlock(&mutex);
        
        int *saldoptr = malloc(sizeof(int));
        *saldoptr = novo_saldo;
        
        pthread_exit(saldoptr);    

    
}

int main() {
    void *status;
    int resultado = 0;    
   
    pthread_t threads[NUM_THREADS];

    //Criar as threads 
    for (int i = 0; i < 5; i++){
        pthread_create(&threads[i], NULL, AtualizaSaldo, (void *)(intptr_t)i+1);     //Esse ultimo argumento, é o ...
    }
    
    //Coletar os resultados para somar o saldo final 
    for (int i = 0; i < 5; i++){
        pthread_join(threads[i], &status);     //Status que vai passar o valor do saldo
        resultado = resultado + *(int *)status;
    }


    printf("Saldo Final: %d\n", resultado);
    return 0;
}
